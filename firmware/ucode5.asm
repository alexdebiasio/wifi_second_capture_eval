...
#define PKT_SEND_TDMA r63
#define RX_IN_PROGRESS r62
#define TX_IN_PROGRESS r61
#define CLI_DELAY   r60
#define SLOT_BITMASK    r59
#define TDMA_COUNTER    r58
#define TDMA_COUNTER_W1 r57
#define TMP r56
#define TMP2 r55
#define START_TDMA r54
...
//TDMA slot init
    mov 0x0FFF, [SHM(0xFF2)]
    mov 0x0080, [SHM(0xFF0)]
...
state_machine_start:
//check if there is something to send with TDMA. if it is the case and manca poco, start spinning, otherwhise goto CLEANBIT
	CLEANBIT(1, SPR_PSM_COND)
    
    mov [SHM(0x0FF0)], CLI_DELAY
    mov [SHM(0x0FF2)], SLOT_BITMASK

    mov [SHM(0x0FF4)], TMP
    je  TMP, 0, skip_reset_counter
    mov 0, TDMA_COUNTER
    mov 0, TDMA_COUNTER_W1
    mov 0, [SHM(0x0FF4)]


skip_reset_counter:

    //if START_TDMA is not raised skip

    je START_TDMA, 0, check_discard_tdma_frames

    je PKT_SEND_TDMA, 0, nothing_to_send_w_tdma

    // extract value from timer and check if it is time to send
    mov SPR_TSF_WORD0, TMP
    and SLOT_BITMASK, TMP, TMP
    jg  TMP, CLI_DELAY, nothing_to_send_w_tdma
    add TMP, 0x20, TMP
    jl  TMP, CLI_DELAY, nothing_to_send_w_tdma


    // trasmission condition (4.d lab5)
    jext COND_NEED_RESPONSEFR, nothing_to_send_w_tdma
    jnand   SPR_TXE0_CTL,   TXE0_SCHEDULE_WORKING,  nothing_to_send_w_tdma
    je RX_IN_PROGRESS, 1 ,nothing_to_send_w_tdma
    je TX_IN_PROGRESS, 1 ,nothing_to_send_w_tdma
    jext  COND_TX_NOW, nothing_to_send_w_tdma 
...
keep_spinning:
    mov SPR_TSF_WORD0, TMP
    and SLOT_BITMASK, TMP, TMP
    jge TMP, CLI_DELAY, tx_immediately
    jext COND_TRUE, keep_spinning

tx_immediately:

// Edit on the fly the receiver addr BB:AA:DD:CC:FF:EE
    mov  0xBBAA, SPR_TME_VAL10 
    mov  0xDDCC, SPR_TME_VAL12 
    mov  0xFFEE, SPR_TME_VAL14 
    mov  0xFFFF, SPR_TME_MASK10
    mov  0xFFFF, SPR_TME_MASK12
    mov  0xFFFF, SPR_TME_MASK14 
    or  SPR_TXE0_WM0,  0xE0, SPR_TXE0_WM0  


    add. TDMA_COUNTER, 1, TDMA_COUNTER
    addc TDMA_COUNTER_W1, 0, TDMA_COUNTER_W1
    mov 0, SPR_BRC
    orx 7,  8,  0x000,   0x004,  SPR_RXE_FIFOCTL1
    or  SPR_RXE_FIFOCTL1,   0x000,  REG34
    mov 0x4007, SPR_TXE0_CTL
    mov 0, PKT_SEND_TDMA
    mov 1, TX_IN_PROGRESS
    jext COND_TRUE, tx_data

check_discard_tdma_frames:
    je PKT_SEND_TDMA,0, nothing_to_send_w_tdma
    mov 0, PKT_SEND_TDMA
    jext COND_TRUE, suppress_this_frame


nothing_to_send_w_tdma:
...
//check if i should send in TDMA or in DCF fashion
check_tx_next_txe_ctl_2:
    mov [TXHDR_UNUSED,off0], PKT_SEND_TDMA
    jne     PKT_SEND_TDMA, 0, pkt_to_send_with_tdma
	jext	COND_TRUE, set_ifs

pkt_to_send_with_tdma:
...
//
send_response:
    //clear PKT_SEND_TDMA because I must send an ACK with DCF
    mov 0, PKT_SEND_TDMA
...
rx_bss_match:
    ...
    //If is a beacon with vendor field set to "bs" set START_TDMA to 0

    je START_TDMA, 0, vendor_field_check
    mov [SHM(0xA3C)], TMP
    mov 0x7362, TMP2 //0x7361 is "bs" in ASCII
    jne TMP, TMP2, vendor_field_check
    mov 0, START_TDMA


vendor_field_check:

    //If is a beacon with vendor field set to "as" raise START_TDMA
    
    je START_TDMA, 1, skip_raise_start_tdma
    
    mov [SHM(0xA3C)], TMP
    mov 0x7361, TMP2 //0x7361 is "as" in ASCII
    jne TMP, TMP2, skip_raise_start_tdma
    mov 1, START_TDMA

skip_raise_start_tdma:
